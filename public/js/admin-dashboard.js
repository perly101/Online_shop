...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (storedOrders) {
        try {
            ...
            ordersData = ordersData.map(order => {
                ...
                // Recalculate total based on fixed item prices and quantities
                if (order.items) {
                    order.total = order.items.reduce((sum, item) => {
                        // Handle both 'quantity' and 'qty' fields for compatibility
                        const quantity = item.quantity || item.qty || 1;
                        const price = parseFloat(item.price) || 0;
                        return sum + (price * (parseInt(quantity) || 1));
                    }, 0);
                }
    ...
    function renderOrders() {
        ...
        recentOrders.forEach((order, index) => {
            ...
            orderItem.innerHTML = `
                ...
                <div class="order-detail">
                    <strong>Customer Name:</strong> ${order.customerName || order.customerEmail || 'Unknown Customer'}
                </div>
                <div class="order-detail">
    ...
    function sendNotificationForStatusChange(order, newStatus) {
        ...
        // Also store notification for the specific customer only
        try {
            // Create a user-specific notifications key based on customer email
            const customerEmail = order.customerEmail || 'unknown';
            const userNotificationsKey = `notifications_${customerEmail}`;
            
            const raw = localStorage.getItem(userNotificationsKey) || '[]';
            let notifications = [];
            ...
            // Note: We don't trigger updateCustomerNotificationDot() here as this runs in admin context
            // The customer UI will check for notifications when they load their notifications panel
        } catch (e) {
        ...
        if (storedOrders) {
            try {
                ...
                if (JSON.stringify(newOrders) !== JSON.stringify(ordersData)) {
                    console.log('Orders data changed, updating display');
                    ordersData = newOrders;
                    renderOrders();
                    updateOrderBadge(); // Update the order count badge
                } else {
                    console.log('No changes in orders data');
                }
            } catch (e) {
                console.error('Error parsing stored orders data:', e);
            }
    ...
    console.log('Initial sync of customer orders to admin orders');
    syncCustomerOrdersToAdmin();
    ...
    function syncCustomerOrdersToAdmin() {
        try {
            ...
            console.log('Syncing orders - Customer orders count:', customerOrders.length, 'Admin orders count:', adminOrders.length);
            
            // Process each customer order
            let newOrdersAdded = false;
            customerOrders.forEach(customerOrder => {
                // Check if this order already exists in admin orders by ID
                const existingOrder = adminOrders.find(order => order.id == customerOrder.id);
                
                if (existingOrder) {
                    console.log('Order already exists in admin orders:', customerOrder.id);
                    return;
                }
    ...
    function convertCustomerOrderToAdminFormat(customerOrder) {
        ...
        if (customerOrder.contact) {
            customerName = customerOrder.contact.name || customerOrder.contact.email || 'Unknown Customer';
            customerPhone = customerOrder.contact.mobile || 'N/A';
            customerEmail = customerOrder.contact.email || customerOrder.contact.name || 'N/A';
        }
        
        // Calculate total from items to ensure accuracy
        let total = 0;
        if (customerOrder.items && Array.isArray(customerOrder.items)) {
            total = customerOrder.items.reduce((sum, item) => {
                // Ensure proper type conversion for qty and price
                const qty = typeof item.qty === 'number' && !isNaN(item.qty) ? item.qty : 
                           (typeof item.qty === 'string' && !isNaN(parseFloat(item.qty))) ? parseFloat(item.qty) : 1;
                const price = typeof item.price === 'number' && !isNaN(item.price) ? item.price : 
